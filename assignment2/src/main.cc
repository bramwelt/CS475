...
main( int argc, char *argv[ ] )
{
    ...
    for( int t = 0; t < NUMSTEPS; t++ )
    {
        ...
        for( int i = 0; i < NUMBODIES; i++ )
        {
            ...
            #ifndef CORSE // Fine-grained parallelism
            //printf("FINE-");
            #ifdef DYNAMIC
            //printf("DYNAMIC\n");
            #pragma omp parallel for default(none), shared(Bodies, i, bi), reduction(+:fx,fy,fz), schedule(dynamic)
            #else
            //printf("STATIC\n");
            #pragma omp parallel for default(none), shared(Bodies, i, bi), reduction(+:fx,fy,fz), schedule(static)
            #endif // DYNAMIC
            #endif // CORSE
            //#pragma omp parallel for default(none), shared(Bodies, i, bi), reduction(+:fx,fy,fz)
...
#if NUMT == 1
    cout << "NUMBODIES\tNUMSTEPS\tNUMT\tMBCPs\tTIME" << "\n";
#endif
    cout << NUMBODIES << "\t";
    cout << NUMSTEPS << "\t";
    cout << NUMT << "\t";
    cout << (float)NUMBODIES*(float)NUMBODIES*(float)NUMSTEPS/timedelta/1000000. << "\t";
    cout << timedelta << "s";
    cout << "\n";
