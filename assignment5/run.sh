...
for G in ${global_size[@]}; do
    for L in ${local_size[@]}; do
        ...
        ./first | tee -a data/vector-mult-${G}.tmp
    done
    column -t data/vector-mult-${G}.tmp > data/vector-mult-${G}.dat
    rm data/vector-mult-${G}.tmp
done

for L in ${local_size[@]}; do
    for G in ${global_size[@]}; do
        make -s  USE_GROUP=1 LOCAL_SIZE=${L} NUM_ELEMENTS=$((2**${G}*512))
        ./first | tee -a data/vector-mult-group-${L}.tmp
    done
    column -t data/vector-mult-group-${L}.tmp > data/vector-mult-group-${L}.dat
    rm data/vector-mult-group-${L}.tmp
done
